...
//new \Org\Util\Mydbda;
class BaojialistcghController extends Controller {
	protected $model;
    ...
    public function baojia_list(){
		...
		$Mybj = M('scn_ark_product');
		$sql =  "
		  select * from scn_ark_product where userid = '".$userid."' and ark_type=2 and status<3 order by id desc;
		";
	...
	public function baojia_list_search(){
		...
		$Mybj = M('scn_ark_product');
		$sql =  "select * from scn_ark_product where userid = '".$userid."' and ark_type=2 and concat(cInvName,cInvStd,vendor) like '%$keyword%' order by truetime desc;";
		$data_list = $Mybj->query($sql);
		$this->assign('data_list',$data_list);
		$this->display('baojia_list');
		//var_dump($data_list);
	...
	public function baojia_list_cinvname(){
		...
		//echo $where;
		//exit;
	    $sql =  "select * from scn_ark_product where userid = '".$userid."' and ark_type=2 ".$where." order by truetime desc";
		$data_list = $Mybj->query($sql);
		$this->assign('data_list',$data_list);
		$this->display('baojia_list');
		//var_dump($data_list);
	...
	public function baojia_list_count(){
		$userid = I('post.userid');
		$Mybj = M('scn_ark_product');
		$sql =  "select * from scn_ark_product where userid = '".$userid."' and ark_type=2 and status<3";
	...
	public function baojia_list_count2(){
		$userid = I('post.userid');
		$Mybj = M('scn_ark_product');
		$sql =  "select * from scn_ark_product where userid = '".$userid."' and ark_type=2 and status=0";
        $result=$Mybj->query($sql);
        echo $data=count($result);
	}
	...
	public function baojia_user_search($id){
		...
		$Mybj = M('scn_ark_product');
		$sql = "
		  select * from scn_ark_product where id = ".$id.";
		";
        $data_list = $Mybj->query($sql);
		return $data_list;
        //var_dump($data_list);		
