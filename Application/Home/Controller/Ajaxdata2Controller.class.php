...
class Ajaxdata2Controller extends Controller {
	...
	public function ajax_list_cInvName(){
		...
		$sql = "
           ...
           (select cInvName,userid from scn_ecms_inventoryexisting_xgxhbj_product group by cInvName ,userid order by cInvName ASC)
	...
	public function ajax_list_vendor(){
        ...
        $xinghao = M('scn_ecms_inventoryexisting_xgxhbj_xinghao');
        if($cInvName=="全部"){
           $where['cInvName'] ="POM";
        }else{
