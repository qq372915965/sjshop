...
//new \Org\Util\Mydbda;
class BaojialistxgController extends Controller {
	protected $model;
	
    public function baojia_list(){
		$userid = I('post.userid');
		
		$Mybj = M('scn_ecms_inventoryexisting_xgxhbj_product');
		$sql =  "
		  select * from scn_ecms_inventoryexisting_xgxhbj_product where userid = '".$userid."' and status<3 order by id desc;
		";
        ...
        if($shijian<=0){
        	$Mybj2 = M('scn_ecms_inventoryexisting_xgxhbj_product');
        	$s_data['status']=1;
	...
	/*public function baojia_list_search(){
		...
		$Mybj = M('scn_ecms_inventoryexisting_xgxhbj_product');
		$sql =  "select * from scn_ecms_inventoryexisting_xgxhbj_product where userid = '".$userid."' and concat(cInvName,cInvStd,vendor) like '%$keyword%' order by truetime desc;";
		$data_list = $Mybj->query($sql);
		$this->assign('data_list',$data_list);
		$this->display('baojia_list');
		//var_dump($data_list);
	...
	public function baojia_list_cinvname(){
		...
		//echo $where;
		//exit;
	    $sql =  "select * from scn_ecms_inventoryexisting_xgxhbj_product where userid = '".$userid."' ".$where." order by truetime desc";
		$data_list = $Mybj->query($sql);
		$this->assign('data_list',$data_list);
		$this->display('baojia_list');
		//var_dump($data_list);
	...
	public function baojia_list_count(){
		$userid = I('post.userid');
		$Mybj = M('scn_ecms_inventoryexisting_xgxhbj_product');
		$sql =  "select * from scn_ecms_inventoryexisting_xgxhbj_product where userid = '".$userid."' and status<3";
	...
	public function baojia_list_count2(){
		$userid = I('post.userid');
		$Mybj = M('scn_ecms_inventoryexisting_xgxhbj_product');
		$sql =  "select * from scn_ecms_inventoryexisting_xgxhbj_product where userid = '".$userid."' and status=0";
        $result=$Mybj->query($sql);
        echo $data=count($result);
	}
	...
	public function baojia_user_search($id){
		...
		$Mybj = M('scn_ecms_inventoryexisting_xgxhbj_product');
		$sql = "
		  select * from scn_ecms_inventoryexisting_xgxhbj_product where id = ".$id.";
		";
        $data_list = $Mybj->query($sql);
		return $data_list;
        //var_dump($data_list);		
	...
	public function baojia_save_add($data){
        ...
        $baojia = $Mybj->query($sql);	
           ...
           $data =  $baojia[0];
		 ***/
		$Mybj = M('scn_ecms_inventoryexisting_xgxhbj_cache');
	    $Mybj ->add($data);
		//$Mybj->addAll($data);
