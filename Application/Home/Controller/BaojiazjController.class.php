...
//new \Org\Util\Mydbda;
class BaojiazjController extends Controller {
	protected $model;
 ...
 ///////////////////////////助剂/////////////////////////////////////
     ...
     public function baojiazj_list(){  //助剂

        $userid = I('post.userid');
     	$status = I('post.grade');
		$Mybj = M('scn_ecms_aids_product');
		...
		$this->display('baojiazj_list');
		//var_dump($data_list);
	}



	public function baojiazj_list_save(){
		$status = I('get.status');
		$data = I('post.data');
		...
		*/
		...
		foreach($data as $key => $value){
           ...
           $baojia = $this -> baojiazj_user_search($value['id']);	
		   //保存USERID
           ...
           //$dataList[] = $baojia[0];
           $this ->baojiazj_save_add($baojia[0]);	
   		   //var_dump($baojia[0]);
		}
		...
		$Mybj = M('scn_ecms_aids_product');
		$sql =  "
		  select * from scn_ecms_aids_product where userid = '".$userid."' and status<3 order by id desc;
		";
	...
	//传Id查询报价
	public function baojiazj_user_search($id){
		//$id = I('get.id');
		$Mybj = M('scn_ecms_aids_product');
		$sql = "
		  select * from scn_ecms_aids_product where id = ".$id.";
		";
        $data_list = $Mybj->query($sql);
		return $data_list;
        //var_dump($data_list);		
	...
	//插入单条数据
	public function baojiazj_save_add($data){

		$Mybj = M('scn_ecms_aids_cache');
