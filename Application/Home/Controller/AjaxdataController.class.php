...
class AjaxdataController extends Controller {
	...
	public function ajax_saerch_baojia_count(){
		...
		$where['baojia']=1;
		$where['validity']=0;*/
		$where="second=0 and status=0 and baojia=1 and validity=0 and userfen=1 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600";
	...
	public function ajax_saerch_baojia(){
		...
		if(($cInvName)&&($cInvName != '全部')){
			...
			$sql="SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,
					...
					vendor,companyhome,address,truetime,baojiacount,userid,companyconcept,
					SECOND,userfen,STATUS,baojia,validity,fprice 
					FROM scn_ecms_inventoryexisting_dlxhbj_product where ".$where." 
					and SECOND=0 AND userfen=1 AND STATUS=0 AND baojia=1 AND validity=0 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600 
					ORDER BY id desc )
					 AS tabA  
			...
			$sql="SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,
					...
					vendor,companyhome,address,truetime,baojiacount,userid,companyconcept,
					SECOND,userfen,STATUS,baojia,validity,fprice 
					FROM scn_ecms_inventoryexisting_dlxhbj_product WHERE SECOND=0 AND userfen=1 AND STATUS=0 AND baojia=1 AND validity=0 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600 ORDER BY id desc )
					 AS tabA  
					GROUP BY cinvname,cinvstd,vendor,address ORDER BY prconut DESC limit 0,200";
            $data=$xinghao->query($sql);
			//$data = $xinghao->where($where)->group('cInvName,vendor,cInvStd,address')->limit(0,200)->select();
	...
	public function ajax_saerch_baojia_city(){
		...
		$xinghao = M('scn_ecms_inventoryexisting_dlxhbj_product');
		if($city){
			 $where = "( companyhome regexp '".$city."'  )";
		}else{
			 $where = '';
		}
		if($city=="东莞其他"){
             $where = "( companyhome regexp '东莞' and companyhome not regexp '樟木头|黄江|常平' )";
		}
		if($city=="浙江其他"){
             $where = "( companyhome regexp '浙江' and companyhome not regexp '余姚' )";
		}
		if($city=="江苏其他"){
             $where = "( companyhome regexp '江苏' and companyhome not regexp '太仓|昆山' )";
		}
		if($city=="山东其他"){
             $where = "( companyhome regexp '山东' and companyhome not regexp '临沂' )";
		}
	...
	//香港现货
	public function ajax_xgxh_baojia_count(){
		...
		//var_dump($cInvName);
        $where="second=0 and status=0 and baojia=1 and validity=0 and userfen=2 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600";
		if(($cInvName)&&($cInvName != '全部')){
			$where.=" and cInvName='".$cInvName."'";
			if($vendor){
				$where.= " and vendor='".$vendor."'";
	...
	public function ajax_xgxh_baojia(){
		$cInvName = I('post.cInvName');
		$vendor = I('post.vendor');
		$cinvstd = I('post.cinvstd');
        $address = I('post.address');
		...
		if(($cInvName)&&($cInvName != '全部')){
			...
			$sql="SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,
					...
					vendor,companyhome,address,truetime,baojiacount,userid,companyconcept,
					SECOND,userfen,STATUS,baojia,validity,fprice 
					FROM scn_ecms_inventoryexisting_xgxhbj_product where ".$where." ORDER BY id desc )
					 AS tabA  WHERE SECOND=0 AND userfen=2 AND STATUS=0 AND baojia=1 AND validity=0 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600
					GROUP BY cinvname,cinvstd,vendor,address ORDER BY prconut DESC limit 0,200";
            $data=$xinghao->query($sql);
			//$data = $xinghao->where($where)->group('cInvName,vendor,cInvStd,address')->limit(0,200)->select();
			
		}else{
			$sql="SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,companyconcept,
					vendor,companyhome,address,truetime,baojiacount,userid,
					COUNT(*) AS prconut, fprice 
					FROM (SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,companyconcept,
					vendor,companyhome,address,truetime,baojiacount,userid ,
					SECOND,userfen,STATUS,baojia,validity,fprice 
					FROM scn_ecms_inventoryexisting_xgxhbj_product ORDER BY id desc )
					 AS tabA  WHERE SECOND=0 AND userfen=2 AND STATUS=0 AND baojia=1 AND validity=0 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600
					GROUP BY cinvname,cinvstd,vendor,address ORDER BY prconut DESC limit 0,200";
            $data=$xinghao->query($sql);
			//$data = $xinghao->where($where)->group('cInvName,vendor,cInvStd,address')->limit(0,200)->select();
				...
				if($minute<1){
				 ...
				 if($minute<1440&&$minute>60){
				  ...
				  if($minute>1440){
                    ...
                    <td>
                       <a href="/sjshop/index.php/Home/Baojialist/baojialist_xgxh_mx?cInvName='.urlencode($cInvName).'&vendor='.urlencode($vendor).'&cInvStd='.urlencode($cInvStd).'&address='.urlencode($address).'&paixu=truetime desc" class="xd2" target="_blank">'.$baojiacount.'</a>
	...
	//香港期货
	public function ajax_xgqh_baojia_count(){
		...
		//var_dump($cInvName);
        $where="second=0 and status=0 and baojia=1 and validity=0 and userfen=3 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600";
		if(($cInvName)&&($cInvName != '全部')){
			$where.=" and cInvName='".$cInvName."'";
			if($vendor){
				$where.= " and vendor='".$vendor."'";
	...
	public function ajax_xgqh_baojia(){
		$cInvName = I('post.cInvName');
		$vendor = I('post.vendor');
		$cinvstd = I('post.cinvstd');
        $address = I('post.address');
		...
		if(($cInvName)&&($cInvName != '全部')){
			...
			$sql="SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,
					...
					vendor,companyhome,address,truetime,baojiacount,userid,companyconcept,
					SECOND,userfen,STATUS,baojia,validity,fprice 
					FROM scn_ecms_inventoryexisting_xgqhbj_product where ".$where." ORDER BY id desc )
					 AS tabA  WHERE SECOND=0 AND userfen=3 AND STATUS=0 AND baojia=1 AND validity=0 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600
					GROUP BY cinvname,cinvstd,vendor,address ORDER BY prconut DESC limit 0,200";
            $data=$xinghao->query($sql);
			//$data = $xinghao->where($where)->group('cInvName,vendor,cInvStd,address')->limit(0,200)->select();
			...
			$sql="SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,
					...
					vendor,companyhome,address,truetime,baojiacount,userid,companyconcept,
					SECOND,userfen,STATUS,baojia,validity,fprice 
					FROM scn_ecms_inventoryexisting_xgqhbj_product ORDER BY id desc )
					 AS tabA  WHERE SECOND=0 AND userfen=3 AND STATUS=0 AND baojia=1 AND validity=0 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600
					GROUP BY cinvname,cinvstd,vendor,address ORDER BY prconut DESC limit 0,200";
            $data=$xinghao->query($sql);
			//$data = $xinghao->where($where)->group('cInvName,vendor,cInvStd,address')->limit(0,200)->select();
				...
				if($minute<1){
				 ...
				 if($minute<1440&&$minute>60){
				  ...
				  if($minute>1440){
                    ...
                    <td>
                       <a href="/sjshop/index.php/Home/Baojialist/baojialist_xgqh_mx?cInvName='.urlencode($cInvName).'&vendor='.urlencode($vendor).'&cInvStd='.urlencode($cInvStd).'&address='.urlencode($address).'&paixu=truetime desc" class="xd2" target="_blank">'.$baojiacount.'</a>
                   </td>
                   </tr>
					';		
		}
    ...
    //报价搜索
    public function saerch_baojia_text_count(){
		
		$cInvName = I('post.cInvName');
		$vendor = I('post.vendor');
		$cinvstd = I('post.cinvstd');
        $address = I('post.address');
        $keyboard = I('post.keyboard');
		...
		$classid = $this->CheckModel->CheckStr($classid);
		$xinghao = M('scn_ecms_inventoryexisting_dlxhbj_product');
		$xinghao2 = M('scn_ecms_inventoryexisting_xgxhbj_product');
		$xinghao3 = M('scn_ecms_inventoryexisting_xgqhbj_product');
		/*$where['second']=0;
		$where['userfen']=1;
		$where['status']=0;
		$where['baojia']=1;
		...
		if(($cInvName)&&($cInvName != '全部')){
			$where ="second=0 and status=0 and baojia=1 and validity=0 and cInvName='".$cInvName."'";
			if($vendor){
				$where.= " and vendor='".$vendor."'";
			}
			if($cinvstd){
			...
			if($classid==1){			
			echo $data = $xinghao->where($where." and userfen=1")->count();
		    }else if($classid==2){
		    	echo $data = $xinghao2->where($where." and userfen=2")->count();
		    }else if($classid==3){
                echo $data = $xinghao2->where($where." and userfen=3")->count();
		    } 
			
		}else{
			if($classid==1){
			echo $data = $xinghao->where(" status=0 and baojia=1 and validity=0 and userfen=1 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600 and concat(cInvName,cInvStd,vendor,companyreferred) like '%".$keyboard."%'")->count();
		    }else if($classid==2){
                echo $data = $xinghao2->where(" status=0 and baojia=1 and validity=0 and userfen=2 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600 and concat(cInvName,cInvStd,vendor,companyreferred) like '%".$keyboard."%'")->count();
		    }else if($classid==3){
		    	echo $data = $xinghao3->where(" status=0 and baojia=1 and validity=0 and userfen=3 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600 and concat(cInvName,cInvStd,vendor,companyreferred) like '%".$keyboard."%'")->count();
		    }
		}
        //var_dump( $xinghao ->getLastSql());
		//exit;
	...
	public function saerch_baojia_text(){
		$cInvName = I('post.cInvName');
		$vendor = I('post.vendor');
		$cinvstd = I('post.cinvstd');
        $address = I('post.address');
		...
		if(($cInvName)&&($cInvName != '全部')){
			...
			$sql="SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,
					...
					vendor,companyhome,address,truetime,baojiacount,userid,companyconcept,
					SECOND,userfen,STATUS,baojia,validity,fprice 
					FROM scn_ecms_inventoryexisting_dlxhbj_product where ".$where." ORDER BY  id  desc )
					 AS tabA  WHERE SECOND=0 AND userfen=1 AND STATUS=0 AND baojia=1 AND validity=0 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600
					GROUP BY cinvname,cinvstd,vendor,address ORDER BY prconut DESC limit 0,200";
				}else if($classid==2){
			$sql="SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,
					vendor,companyhome,address,truetime,baojiacount,userid,companyconcept,
					COUNT(*) prconut, fprice 
					FROM (SELECT id, companyreferred,thecontact,companyname, contactphone,cinvname,cinvstd,
					vendor,companyhome,address,truetime,baojiacount,userid,companyconcept,
					SECOND,userfen,STATUS,baojia,validity,fprice 
					FROM scn_ecms_inventoryexisting_xgxhbj_product where ".$where." ORDER BY  id desc )
			...
			if($classid==1){
			$sql="select id,title,cInvName,cInvStd,companyname, fPrice,dValidDate,fWeight,cAddress,cPartnerCode,vendor,companyid,second,Thecontact,Contactphone,truetime,companyhome,userid,address,companyconcept,companyreferred,count(*) prconut
			    from (SELECT id,title,cInvName,cInvStd,companyname,fPrice,dValidDate,fWeight,cAddress,cPartnerCode,vendor,companyid,second,Thecontact,Contactphone,truetime,companyhome,userid,address,companyconcept,companyreferred,userfen,STATUS,baojia,validity FROM scn_ecms_inventoryexisting_dlxhbj_product where concat(cInvName,cInvStd,vendor,companyreferred) like '%$keyboard%' ORDER BY id desc)
			    AS tabA  WHERE userfen=1 AND STATUS=0 AND baojia=1 AND validity=0 and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600
			    GROUP BY cinvname,cinvstd,vendor,address ORDER BY prconut DESC limit 200";
			   }else if($classid==2){
			$sql="select id,title,cInvName,cInvStd,companyname, fPrice,dValidDate,fWeight,cAddress,cPartnerCode,vendor,companyid,second,Thecontact,Contactphone,truetime,companyhome,userid,address,companyconcept,companyreferred,count(*) prconut
			    from (SELECT id,title,cInvName,cInvStd,companyname,fPrice,dValidDate,fWeight,cAddress,cPartnerCode,vendor,companyid,second,Thecontact,Contactphone,truetime,companyhome,userid,address,companyconcept,companyreferred,userfen,STATUS,baojia,validity FROM scn_ecms_inventoryexisting_xgxhbj_product where concat(cInvName,cInvStd,vendor,companyreferred) like '%$keyboard%' ORDER BY id  desc)
			    AS tabA  WHERE userfen=2 AND STATUS=0 AND baojia=1 AND validity=0
			    and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600
			    GROUP BY cinvname,cinvstd,vendor,address ORDER BY prconut DESC limit 200";
			   }else if($classid==3){
			$sql="select id,title,cInvName,cInvStd,companyname, fPrice,dValidDate,fWeight,cAddress,cPartnerCode,vendor,companyid,second,Thecontact,Contactphone,truetime,companyhome,userid,address,companyconcept,companyreferred,count(*) prconut
			    from (SELECT id,title,cInvName,cInvStd,companyname,fPrice,dValidDate,fWeight,cAddress,cPartnerCode,vendor,companyid,second,Thecontact,Contactphone,truetime,companyhome,userid,address,companyconcept,companyreferred,userfen,STATUS,baojia,validity FROM scn_ecms_inventoryexisting_xgqhbj_product where concat(cInvName,cInvStd,vendor,companyreferred) like '%$keyboard%' ORDER BY id  desc)
			    AS tabA  WHERE userfen=3 AND STATUS=0 AND baojia=1 AND validity=0
			    and (UNIX_TIMESTAMP(NOW())-truetime)<15*24*3600
			    GROUP BY cinvname,cinvstd,vendor,address ORDER BY prconut DESC limit 200";
			   }
            $data=$xinghao->query($sql);
				...
				if($minute<1){
				 ...
				 if($minute<1440&&$minute>60){
				  ...
				  if($minute>1440){
                    ...
                    if($classid==1){
                    if($second==0){ 
                      echo '<a href="/e/action/gongying_zx.php?cInvName='.$cInvName.'&vendor='.$vendor.'&cInvStd='.$cInvStd.'&address='.$address.'&paixu=truetime desc" class="xd2">'.$baojiacount.'</a>';
                    }else{
                      echo '<a href="/e/action/gongying_zxfp.php?cInvName='.$cInvName.'&vendor='.$vendor.'&cInvStd='.$cInvStd.'&address='.$address.'&paixu=truetime desc" class="xd2">'.$baojiacount.'</a>';
                    }
                    }else if($classid==2){
                    if($second==0){ 	
                      echo '<a href="/e/action/gongying_xg.php?cInvName='.$cInvName.'&vendor='.$vendor.'&cInvStd='.$cInvStd.'&address='.$address.'&paixu=truetime desc" class="xd2">'.$baojiacount.'</a>';
                    }else{
                      echo '<a href="/e/action/gongying_xgfp.php?cInvName='.$cInvName.'&vendor='.$vendor.'&cInvStd='.$cInvStd.'&address='.$address.'&paixu=truetime desc" class="xd2">'.$baojiacount.'</a>';
                    }  
                    }else if($classid==3){
                    if($second==0){ 
                      echo '<a href="/e/action/gongying_xg2.php?cInvName='.$cInvName.'&vendor='.$vendor.'&cInvStd='.$cInvStd.'&address='.$address.'&paixu=truetime desc" class="xd2">'.$baojiacount.'</a>'; 
                    }else{
                      echo '<a href="/e/action/gongying_xg2.php?cInvName='.$cInvName.'&vendor='.$vendor.'&cInvStd='.$cInvStd.'&address='.$address.'&paixu=truetime desc" class="xd2">'.$baojiacount.'</a>';
                    }  
                   '</td>
