...
//new \Org\Util\Mydbda;
class BaojialistghController extends Controller {
	protected $model;
    ...
    public function baojia_list(){
		...
		$sql =  "
		  select * from scn_ark_product where userid = '".$userid."' and ark_type=1 and status<3 order by id desc;
		";
	...
	public function baojia_list_search(){
		...
		$Mybj = M('scn_ark_product');
		$sql =  "select * from scn_ark_product where userid = '".$userid."' and ark_type=1 and concat(cInvName,cInvStd,vendor) like '%$keyword%' order by truetime desc;";
		$data_list = $Mybj->query($sql);
		$this->assign('data_list',$data_list);
		$this->display('baojia_list');
		//var_dump($data_list);
	...
	public function baojia_list_cinvname(){
		...
		//echo $where;
		//exit;
	    $sql =  "select * from scn_ark_product where userid = '".$userid."' and ark_type=1 ".$where." order by truetime desc";
		$data_list = $Mybj->query($sql);
		$this->assign('data_list',$data_list);
		$this->display('baojia_list');
		//var_dump($data_list);
	...
	public function baojia_list_count(){
		$userid = I('post.userid');
		$Mybj = M('scn_ark_product');
		$sql =  "select * from scn_ark_product where userid = '".$userid."' and ark_type=1 and status<3";
	...
	public function baojia_list_count2(){
		$userid = I('post.userid');
		$Mybj = M('scn_ark_product');
		$sql =  "select * from scn_ark_product where userid = '".$userid."' and ark_type=1 and status=0";
        $result=$Mybj->query($sql);
        echo $data=count($result);
	}
