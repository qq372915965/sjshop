		...
		if(confirm("是否要保存该数据？")){
			...
			obj.thecontact=$(".thecontact",p).val();
			  ...
			  //alert(JSON.stringify(list));
			  			 $.post("/weixin/index.php/Home/Baojialistgh/baojia_list_save",
	...
	$(".baojia-list-cont table tbody").on("click"," .up",function(){
		...
		if(confirm("是否要上架该数据？")){	
			...
			obj.thecontact=$(".thecontact",p).val();
			  ...
			  list.push(obj);
			  //alert(JSON.stringify(list));
			  			 $.post("/weixin/index.php/Home/Baojialistgh/baojia_list_save/status/0",
							 {data:JSON.stringify(list)},
							 function(data){ 
							  alert('上架成功');
							  $("#baojia_data_list").html(data); 
	...
	$(".baojia-list-cont table tbody").on("click"," .sapan",function(){
		...
		if(confirm("是否要下架该数据？")){
			...
			obj.thecontact=$(".thecontact",p).val();
			  ...
			  list.push(obj);
			  //alert(JSON.stringify(list));
			  			 $.post("/weixin/index.php/Home/Baojialistgh/baojia_list_save/status/1",
							 {data:JSON.stringify(list)},
							 function(data){ 
							  alert('下架成功');
							  $("#baojia_data_list").html(data); 
	...
	$(".baojia-list-cont table tbody").on("click"," .del",function(){
		...
		if(confirm("是否要删除该数据？")){
			...
			obj.thecontact=$(".thecontact",p).val();
			  ...
			  list.push(obj);
			  //alert(JSON.stringify(list));
			  			 $.post("/weixin/index.php/Home/Baojialistgh/baojia_list_save/status/3",
							 {data:JSON.stringify(list)},
							 function(data){ 
							  alert('删除成功');
							  $("#baojia_data_list").html(data); 
